...
(1, "1.4  Contributions", [
    ...
    "(b) Cycle models for both datapaths, and the two are NOT equally strong -- say which is which. The dense one is DERIVED by counting its state machine: (N/P) x T x (9 C_IN + tail) cycles, where N/P is the number of output groups, 9 C_IN the taps per neuron from the 3x3 x C_IN MAC loop, and tail the post-MAC states (4 before C0054, 2 after). It reproduces all six layer-dataset pairs exactly and predicts configurations never built. The event-driven one is part derived, part FITTED: the 2NT sweep floor follows from the two-cycle pipelined beat (C0030), but the 5.0 per-spike queue term and the per-spike scatter constant are regression coefficients whose value drifts ~4 % across layers. The familiar 88 N/P and 71.7 s/K are both C1 special cases. Within 0.3 % of simulation and ~1 % of silicon on two datasets and three layers (experiments/latency_sim/, experiments/dvsgesture/latency_sim/README.md, experiments/rate_sweep*/README.md).",
...
(1, "3.1  FPGA Accelerators for Spiking Neural Networks", [
    ...
    "TAB: [exists] The baseline table (docs/baseline_table.md): work, platform, network/dataset, accuracy, LUT/FF/BRAM/DSP, clock, latency, power method, energy per inference; add a 'this thesis' row at the bottom (and note the two 'ours' rows currently there are SUPERSEDED pass 1-2 figures)."
    "RESULT (throughput per synapse, added 2026-09-20, docs/baseline_table.md final section): the raw latencies in the table are not comparable across rows, so the fabric-independent measure is synaptic operations per cycle -- one weight accumulated into one neuron. Ours: dense 3.60 per cycle at P=4 (0.36 GSOP/s at 100 MHz), event-driven 1.19 useful per cycle at K=4 while performing only 24 % of the accumulates for the same output. **Of eight published designs only one (Minitaur, 0.065 per cycle on MNIST) reports enough to compute this at all**; the rest give a latency for a network whose evaluated-synapse count is unstated or activity-dependent and unreported. That is the same gap that makes their energy numbers incomparable, and it belongs in one sentence beside the power-method column. Say explicitly that 3.6 accumulates per cycle is NOT a claim of competitive throughput: one tap per cycle per lane, zero DSPs by construction, smallest Zynq part, 100 MHz -- the claim is a controlled comparison, not a fast accelerator.",
]),
...
(1, "4.2  Neuron Model and Fixed-Point Arithmetic", [
    ...
    "TAB: [todo] Fixed-point formats: signal, width, signedness, scale, overflow behaviour, where set (golden/network.py lines 35-45 and hdl/common/lif_update.v -- note that golden/lif.py is the M0 FLOAT demo and does not implement the shift leak, so it is not the hardware reference).",
]),
...
(1, "5.4  Host Interface: AXI DMA, Bare-Metal Server and Link Protocol", [
    ...
    "The BURST command and what the project calls engine-only ticks (D0022): latency is timed by the ARM's global timer bracketing one DMA round trip (cache flush, S2MM and MM2S transfers, busy-poll, cache invalidate), NOT on the fabric and NOT excluding the DMA; only UART framing and CRC are outside the bracket. The residual is the +8.3 to +29.4 us per-pass constant already decomposed, so say what the number contains rather than calling it engine-only, what the tick counter includes, and the cross-check against wall-clock over a known N.",
