...
def main():
    ...
    for name, npz in (("c1", "m1_weights_int8.npz"), ("g1", "dvsgesture_weights_int8.npz"),
                      ("r1", "r1_student_weights_int8.npz")):
        ...
        if name == "r1":   # the non-P dense variant exists for r1 only
            checks.append(("hdl/dense/conv_layer_r1.v", r"wrom\[\d+\] = 8'h([0-9a-f]{2});", conv_order(w)))
